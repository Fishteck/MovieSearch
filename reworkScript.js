...
const apiKey = '8486e323ff1d52005b54543dda2d8ec4';
const trends = `${apiHost}/3/trending/all/day?api_key=${apiKey}&language=ru`

const apiSearch = (event) => {              //Используется для отправки данных
    ...
    .then(function (output) {
      ...
      output.results.forEach((item) => {
        ...
        let nameItem = item.name || item.title,
          img = item.poster_path === null ? `<img src="./img/2.jpg" class="card-img-top img-fluid img-thumbnail"alt="no poster" ${dataInfo}>` : `<img src="${imgHost}${item.poster_path}"class="card-img-top img-fluid img-thumbnail" alt="${nameItem}"  ${dataInfo}>`,
...
const addEventMedia = () => {                //Событие при нажатии на картинку и вызов showFullInfo
